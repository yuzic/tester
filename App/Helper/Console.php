...
class Helper_Console
{
    ...
    {
        if (!isset($argv[1]))
            self::usageError('Error class name');
        return $argv;
    }
     ...
     * @return int|boolean Number of bytes printed or false on error
     */
    public static  function stdoutColor($string)
    {
        $args = func_get_args();
        array_shift($args);
        $string = Helper_Console::ansiFormat($string, $args);

        return Helper_Console::stdout($string);
    }
